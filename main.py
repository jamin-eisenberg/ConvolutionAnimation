...
def get_nested_func_range(f, x_opts):
    f_min, f_max = get_func_range(f(x_opts[0]), x_opts)
    for t in frange(*x_opts):
        l_min, l_max = get_func_range(f(t), x_opts)
        f_min = min(f_min, l_min)
        f_max = max(f_max, l_max)
    return f_min, f_max
...
class AnimatedConvolution(Scene):
    ...
    def construct(self):
        ...
        # get the range with a swept value of t
        g2_range = get_nested_func_range(lambda t: lambda tau: self.eq2(t - tau), self.x_opts)
        ...
        prod_range = get_nested_func_range(lambda t: modulate(self.eq1, lambda tau: self.eq2(t - tau)), self.x_opts)
        ...
        ax_conv, graph_conv = plot(convolution_func, PURPLE, self.x_opts,
                                   lambda x: x.move_to(RIGHT * config.frame_width / 4),
...
def main():
    ...
    with tempconfig(
            {"quality": list_get(sys.argv, 8, "medium_quality"), "preview": True, }):
        try:
            x_min = float(sys.argv[5])
            x_max = float(sys.argv[6])
            x_step = float(sys.argv[7])
        except ValueError:
